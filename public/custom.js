...
function initForm() {
  ...
  var paymentArray = profileData ? profileData.paymentType.split(",") : [];
  var options = "<option>Select Payment Type</option>";
  $(paymentArray).each(function (i, key) {
    options = options + "<option>" + key.trim() + "</option>";
  });
  $('#createOrder').find("[name=paymentType]").html(options);
}
...
$(document).ready(function () {
  ...
  function deleteOrders(data, e) {
    var tableId = $(e.target).closest('.tab-pane').find('table').attr('id');
  ...
  if (type == "1") {
    ...
    $.each(data, function (index, value) {
      xlsRows.push({
        ...
        ReferenceNo: value.ref || uniqueId(),
